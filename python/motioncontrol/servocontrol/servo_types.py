...
class ServoConfig:
    ...
    def clamp(self, angle: float) -> float:
        return max(self.min_safe, min(self.max_safe, angle))
    
    def step_down(self, current_angle: float, step_degrees: float) -> float:
        return max(self.min_safe, current_angle - step_degrees) 
    
    def step_up(self, current_angle: float, step_degrees: float) -> float:
        return min(self.max_safe, current_angle + step_degrees)
    
    def from_straight(self, offset: float) -> float:
        return self.clamp(self.straight + offset)
