...
from roboclaw_types import MotorCurveLUT, populate_curve_lut, CurveLUT, populate_lut
...
for i, slope in enumerate(lut_steepness):
    ...
    logi_curve_lut = MotorCurveLUT(min_speed, max_speed, lut_len, slope)
    decay_curve_lut = MotorCurveLUT(min_speed, max_speed, lut_len, slope)
    populate_curve_lut(normalized_logistic_array, logi_curve_lut)
    populate_curve_lut(normalized_decay_array, decay_curve_lut)
