...
def calibration():
    ...
    methods.testing_servo_movement_loop(servo=servodriver, step_degrees=step_degrees, max_angle=300, min_angle=0, set_start_point=s2_params[2], channel=2, debug=True)
    methods.testing_servo_movement_loop(servo=servodriver, step_degrees=step_degrees, max_angle=300, min_angle=0, set_start_point=s3_params[3], channel=3, debug=True)
    methods.servodriver_setzeroes(servodriver=servodriver, s0_pos=s0_params[1], s1_pos=s1_params[1], s2_pos=s2_params[1], s3_pos=s3_params[1])
